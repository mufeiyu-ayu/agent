...
- 只有实现完成且验证通过后，才能把任务文档更新为“已实现、待验收”；Draft、受阻或验证失败时保留原状态、记录阻塞原因，不得进入 GPT 验收。Draft 恢复后必须先完成实现和验证、更新状态，再转为 Ready 接受 Review，不能直接合并。
- 只有 GPT 给出验收通过结论且用户明确确认后，才能把任务写成已通过 / Completed、推进路线或归档阶段。
- 未经用户明确确认，不合并、不自动修复全部 Review findings。
