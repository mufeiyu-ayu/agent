...
当前状态：Phase 1-8 Completed 并归档；当前阶段为源码阅读，先完成当前项目链路学习，之后按 `docs/research/pi-reference/roadmap.md` 由 AI 参照 Pi 实现云端方向；无 Active Task，#118 / #119 / #120 / #124 已于 2026-09-17 合并，#126 / #127 / #115 已于 2026-09-18 合并，#116 / #134 / #135 / #136 已于 2026-09-19 合并（#134 去掉 SEO 产品命名，入口改为 `apps/api/src/chat/` 与 `/api/chat/stream`；#135 删离线评估 baseline 与无运行记录的 smoke，检索评估只剩 `eval:retrieval-quality`；#136 删 tools / agent-runtime / admin 的无消费者字段、重复类型与单实现包装，工具清单收成 `apps/api/src/tools/tool-definitions.ts`），Next 为 #137（2026-09-19 过度设计审计后立项；web_fetch 同日转 Gated；#117 已于 2026-09-18 关闭转 Gated）；翻译质检站已于 #113 删除；下一批候选子系统为 session 事件流与 replay、审批门、compaction、定时任务，候选不等于 Active；Admin Task 4 保持 Planned。
