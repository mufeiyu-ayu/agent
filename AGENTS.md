...
`docs/development-task-plan.md` 记录项目完成路径和业务任务状态，是后续判断“下一步做什么”的主看板；`docs/work-log.md` 记录项目推进、commit 上下文、关键决策和验证结果。
...
当用户要求提交代码、commit 或 git commit 时，优先使用项目内 skill `.codex/skills/git-commit`：提交前检查并按需更新 `docs/development-task-plan.md` 和 `docs/work-log.md`，验证通过后再创建 commit。
