...
- **实现模式**：“完成 Issue #N”默认执行到 Draft PR、验证结果和学习交接。只记录“已实现、待验收”，不把任务标记为 Completed。
...
3. 根据实际结果更新 Issue 对应任务文档中的 checklist、验证结果和 GitHub 交付记录，并设置：
   - `实施状态：已实现`
   - `验收状态：待验收`
4. 实现模式不得把任务或阶段标记为 Completed，不推进下一任务，不归档阶段，也不把 `docs/roadmap.md` 写成已完成。
5. Issue 本身是实施快照；正式状态仍以 `docs/tasks/**` 为准。
6. 不向 `docs/development-task-plan.md` 写新任务。
...
4. 在原 PR 分支 commit 并 push 收口改动；除非用户同时明确授权，PR 仍保持 Draft，不合并。
...
2. 用户明确授权“合并 PR”即同时授权把该 PR 从 Draft 转为 Ready；先转为 Ready，再合并到 `master`。
...
7. Draft 或尚未合并的分支继续保留；放弃任务的分支只有用户确认后才删除。
