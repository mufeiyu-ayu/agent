...
description: 执行本项目 GitHub Issue 的实现、验收收口与合并后分支清理工作流。Use when the user says 完成 Issue #N、读取 Issue #N 并实现、Claude 已确认 Issue #N 验收通过且用户确认收口、合并 PR #N 并清理分支, or asks to submit the current Issue as a PR. Do not use for ordinary learning conversations, inspection-only work, local experiments, Claude 轻量 docs PR, or small changes unless the user explicitly promotes them into an Issue workflow.
---
...
3. 只有实现完成且必要验证通过后，才更新 Issue 对应任务文档中的 checklist、验证结果和 GitHub 交付记录，并设置：
   - `实施状态：已实现`
   - `验收状态：待验收`
4. 实现未完成、验证失败、任务受阻或等待用户确认时，保留任务文档原状态并记录阻塞原因；不得写成“已实现、待验收”，也不得进入 Claude 验收。
...
Claude Code Review 出现问题时，交由 `github-pr-review-fix` 后续处理。当前流程不要求 GitHub Actions，验证以本地结果为准。

## 7. 验收收口

只有用户明确表达“Claude 已确认 Issue #N 验收通过，我也确认通过，请收口任务状态”等同等授权时执行：
...
如果用户已授权 Claude 完成远程收口说明或轻量 docs 更新，本 skill 不重复执行同一远程提交；Claude Code 只处理用户明确要求的本地同步或后续本地工作。
...
2. Ready PR 直接合并到 `master`，正常流程不需要状态转换。PR 若仍为 Draft，则停止合并并返回实现流程：完成实现和验证、更新为“已实现、待验收”、转为 Ready 接受 Review，再等待 Claude 验收和用户合并授权。
3. fast-forward 同步本地 `master`，确认合并内容已落入主分支。
4. 删除远程 Issue 分支；如果 GitHub 已自动删除或 Claude 已删除，则视为已完成，不重复执行。
5. 使用安全删除清理本地 Issue 分支。
6. 禁止强制删除未合并分支；如果安全删除失败，停止并说明原因。
7. 尚未合并的分支继续保留；放弃任务的分支只有用户确认后才删除。

如果用户已授权 Claude 完成远程合并和远程分支删除，本 skill 只负责本地 `master` 同步、本地分支清理或用户明确要求的本地检查；不得重复合并或重复删除远程分支。
...
- 必要验证失败且尚未解释。
- 用户未明确确认 Claude 验收结论，却要求写入 Completed 状态。
- 用户未授权合并或删除未合并分支。
- Claude 已完成远程合并 / 删除时，用户未要求 Claude Code 做本地同步或本地清理。
