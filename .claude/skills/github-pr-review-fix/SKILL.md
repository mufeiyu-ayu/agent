...
description: 获取并处理本项目 GitHub Pull Request 的 Claude Code Review findings，先讲解问题，再按用户确认在本地修复、验证、commit 并 push 回原 PR。Use when the user says 处理 PR #N 的 Review、修复 PR #N 的审查问题、读取远程 review 问题, or asks to continue a PR after automated review. Do not use for creating a new Issue, opening the initial PR, or changing final task acceptance status.
---
...
6. 修改了功能代码或重要边界时，push 后使用 `/code-review` skill 触发复审；仅回复、resolve thread 或 docs 收口时不重复触发 Review。
