...
本 skill 只在用户明确要求时使用；单角色流程不依赖任何远程 Review。读取并解释评论后，直接修复确认为真问题的项并推送到原 PR；不创建新 PR、不合并。合并与任务验收由 `github-issue-workflow` 的验收步骤负责。用户可以缩小范围，例如「只解释不改」「只修第 2 条」。
...
4. 判定标准与 commit 前 `<review 命令>` 一致：确认为真问题的修；无法本地确定性复现、超出 Issue 声明范围、与最新 Issue 决策或项目规范冲突、判断为误报、属于过度 review（推测性风险、为想象中的后续改动加防御、与本任务无关的重构建议）的不修，并在 PR 评论或描述里写明理由（过度 review 一句话即可）。不为技术判断等待用户确认；只有缺少密钥、权限、登录等授权类前提时才中断询问。
