...
4. 第一版不提供 model filter；只允许展示 `requestedModel: string | null`。`null` 表示调用方未显式指定模型，不得解释成某个具体 provider model。
...
- [ ] 多次 `model_sampling` 正确聚合 sampling count 与真实 usage；
- [ ] 多次 `tool_execution` 正确聚合 tool call count；
- [ ] `requestedModel=null` 保持 `null`；
- [ ] ordinary success、tool success、RUNNING、FAILED、ABORTED 都能稳定投影；
- [ ] Run Detail Timeline 按 `AgentStep.sequence` 排序；
- [ ] 当前五类 Step 都只返回 allowlist；
- [ ] unknown future Step 返回 generic projection 且不泄露 raw JSON；
- [ ] Message 只返回用户可见安全字段 / preview；
- [ ] Run 不存在返回 404；
- [ ] 非法 status、page/pageSize、timestamp 被 DTO 拒绝；
- [ ] pageSize 有服务端硬上限；
- [ ] query/status/date/pagination 组合行为确定；
- [ ] Safe Raw Projection 不包含完整 Step `input/output` 或禁止字段；
- [ ] 查询策略不存在明显逐 Run N+1。

## Green：最小实现

- [ ] 建立 `@agent/contracts` Admin Run Contract；
- [ ] 新增 Admin Runs module / controller / service / DTO；
- [ ] 实现只读 Prisma 查询与稳定分页；
...
- [ ] 消除旧 Admin Mock 对服务端语义的反向控制；
- [ ] Controller 不堆积 Prisma / JSON projection 逻辑；
...
- [ ] server pagination / filters / stable ordering 有确定性测试；
- [ ] sampling/tool count 与 token usage 从真实 durable Step 正确派生；
- [ ] `requestedModel=null` 不被伪造成 resolved model；
- [ ] Run Detail 能表达 Phase 6 多 sampling / 多 Tool Trace；
- [ ] 五类已知 Step 有安全 typed projection；
- [ ] unknown future Step 能安全降级；
- [ ] 不返回 prompt、reasoning、完整 Tool 数据、Observation、stack 或 secret；
- [ ] 404 与非法 query 有明确行为；
- [ ] 不修改 Runtime、Prisma schema、外部 Chat / NDJSON 协议；
...
| requested model 与 resolved model 不等价 | 明确字段名 `requestedModel`，`null` 保持未知 |
