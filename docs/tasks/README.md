...
```text
阶段 1-8：Completed
Active Agent Task：#119 历史裁剪合一（实施状态：已实现 / 验收状态：待验收）
Next：#120 抽出 packages/ai（前置 #119）
Planned：#115 模型调用重试 → #116 同轮文本 + 多 Tool Call → #117 Responses API adapter → web_fetch（R0 后、R2 前）
...
| #119 历史裁剪合一 | **Active** | 实施状态：已实现 / 验收状态：待验收。分支 `claude/issue-119-history-trim-single-query`：删分页选择服务与批内二分，历史一次 `findMany({ take: hardLimit })`，首轮由 planner `excludeOldestHistory` 裁剪；`initialContext` 取裁剪前值、`contextPlan` 记删减数；删 `SEO_CHAT_HISTORY_CANDIDATE_BATCH_SIZE`。验证：`@agent/api` typecheck / lint、`test:context`（真实 tokenizer 差分 legacy 68 = planner 68）、`test:tool-loop`、`test:model-stream`、`test:admin-runs`、`test:grounding`、`test:seo-service` 全绿；本机运行服务两轮真实对话 + Admin 投影冒烟通过 |
| #120 抽出 `packages/ai` | Next | 模型客户端、流适配、类型、错误、profile 搬入零 Nest 的包，apps/api 只留壳；`packages/agent` 仍在 R2；前置 #119 |
