...
Active Agent Task：无
Next：#193 首页替换为设计稿（路线外，只动前台展示层）；源码阅读（第 0 档）从异常断点接着读；RAG 清理三步已随 #189 结束
...
| 任务 | 状态 | 说明 |
| --- | --- | --- |
| #193 首页替换为设计稿 Agent for Teams（百分百还原） | Next | 实施状态：未开始 / 验收状态：未验收。桌面 HTML 原稿移植成 `/` 首页并删旧首页，对原稿做像素比对验收；路线外，只动前台展示层 |
