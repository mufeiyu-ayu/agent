...
```text
阶段 1-8：Completed
Active Agent Task：#115 模型调用重试与 Loop 默认上限（已实现、待验收）
Next：#116 同轮文本 + 多 Tool Call
