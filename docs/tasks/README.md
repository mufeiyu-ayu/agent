...
阶段 1-8：Completed
Active Agent Task：无
Next：#134 去 SEO 产品命名 → #135 删离线评估与 smoke 死链 → #136 零行为变化小修 → #137 embedding 重试收敛（2026-09-19 审计后立项，#134 / #135 可并行）
...
| #134 去 SEO 产品命名 | Next | 代码 / 目录 / 路由 / env / 包名 / 系统提示词 / 前端文案改为 Agent 工作台口径，页面结构与样式不动；并入审计的非流式 chat 删除等四条；Prisma 与数据不动 |
| #135 删离线评估与 smoke 死链 | Next | v1 检索 baseline 闭环、无运行记录的 grounded-answer / embedding smoke、重复 npm scripts、空目录、docs 旧路径；零行为变化，净减 ≥ 1,500 行；与 #134 无交集 |
| #136 零行为变化小修 | Planned | tools 契约无消费者字段与准入门、`InitialContextSummary` 测试专用字段与多余 estimate、重复类型、Admin 镜像类型与 allowlist 派生；前置 #134 / #135 |
| #137 embedding 重试收敛 | Planned | Gemini 重试交给 SDK `retryOptions`，`EMBEDDING_*` 三个 env 改常量；同 #115 / #127 决定；前置 #135 |
