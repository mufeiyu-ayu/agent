...
```text
阶段 1-8：Completed
Active Agent Task：#124 `packages/ai` 目录整理（待验收）
Next：#115 模型调用重试（前置 #120 已于 2026-09-17 合并，#124 在其前）
...
| #124 `packages/ai` 目录按 Pi 分层整理 | Active | 实施状态：已实现 / 验收状态：待验收。分支 `claude/issue-124-ai-package-layout`：根目录收成 `types.ts` / `errors.ts` / `config.ts` / `deepseek.ts`，`clients/` 改 `api/` 并按 `openai-completions*` 命名，全部 `git mv` + import 路径修正，`index.ts` 导出集合不变，零运行时行为变化；另修正 5 处研究文档旧路径。验证：删 dist 后 `@agent/ai` typecheck / build 通过、test 37/37，全 workspace typecheck 6/6，api typecheck / lint 通过 |
