...
```text
阶段 1-8：Completed
Active Agent Task：#116 同轮文本 + 多 Tool Call（实施状态：已实现 / 验收状态：待验收，分支 claude/issue-116-multi-tool-call）
Next：web_fetch（前置 #116 验收合并后）
...
| #116 同轮文本 + 多 Tool Call | Active | 实施状态：已实现 / 验收状态：待验收。分支 `claude/issue-116-multi-tool-call`：sampling decision 只按 finishReason 分派，`SamplingDecision.tool_call.calls[]` 与 `assistant_tool_call.calls[]` 承载同轮多 call，顺序执行、每 call 一个 `tool_execution` Step、预算按 call 计数并在执行前整批拒绝；`length` 截断整批不执行、记 `truncated_arguments` 回喂；adapter 第四条不变量加 `length` 例外、`finalize()` 判重复 id；policy 删 `maxToolCalls < maxSamplingRounds`；删 `ChatMessage` / `toModelInputItems`，finalizer 删 `extra_event_after_completion` / `missing_submission`。验证：workspace typecheck 6/6，api / ai / contracts lint，`@agent/ai` test 47/47，`test:tool-loop` + `test:model-stream` 71/71，`test:grounding` 173/173，`test:context` + seo + llm-config 63/63，`test:tools` 86/86，`test:admin-runs` 55/55，AC-09 grep 0 |
