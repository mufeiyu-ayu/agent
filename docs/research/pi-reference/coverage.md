...
固定 checkout 有 **1714 个 Git tracked 文件**，来自 `git ls-tree -r 8a7b0c03dfb702663acafb6dc29f8acaa4ffe391`，清单可用同一命令重新生成。共 11 个主要功能包（SQLite 是 session-backends 下的嵌套包）以及根级工程资源。

文件数量用于交叉查漏，不能代表语义理解、测试覆盖率或逐行审核完成率。

| 功能域 | tracked 数量（含测试/文档/资源） | 阅读证据与结果 |
| --- | ---: | --- |
| agent | 230 | [runtime/session](./modules/runtime-session.md)：old Agent、13状态Drive、Session/Mutation、3 backend、工具/env、compaction、watch、hooks |
| ai | 347 | [model](./modules/model-telemetry-evals.md)：10种wire API与图片入口、40个provider factory分类、auth/catalog、事件/frame、成本/错误 |
| coding-agent | 762 | [产品](./modules/coding-agent-tui.md)、[宿主](./modules/experimental-host.md)：普通与实验入口、资源/扩展/工具、model装配、各I/O mode、进程生命周期 |
| tui | 113 | 产品说明：两套renderer、输入/布局/组件、terminal/native平台适配 |
...
| session-backends | 33 | runtime/session：SQLite adapter/schema/事务/entries/values/ledger/fork/conformance |
| telemetry | 12 | model：契约、Noop/内存、typed schema、conformance、未全接入的区别 |
| evals | 22 | model：会话harness、五类eval、artifact、paired comparisons |
...
| 周边功能 | 各子目录职责、入口、代表实现与消费者 | 不把目录清单冒充每个helper逐行安全审计 |
| 测试源码 | 关键用例读到触发和断言，其余按场景/文件索引登记 | 阅读断言不是测试通过 |
| 离线断言 | 2026-09-15 执行过 model/wire study checks，直接调用当时源码；脚本已于 2026-09-16 删除，结果记在 verification | 仅证明当时列出的断言，不代替原仓全suite |
| 图表 | 保留8组正式JSON/HTML（第7张为普通 CLI 主链，第8张为生命周期与可定制点）；校验与检查结果记录在verification，临时附件已清理 | 图的美观/绿色校验不能证明架构结论正确；另外交叉核对调用事实 |
...
Delta/CBOR/Unix 的逐功能记录在 `modules/wire-delta-coverage.md`；产品、模型、runtime 的分模块阅读证据表已于 2026-09-16 删除，以各 modules 正文为准。
...
因此本次研究的交付结论是：全仓功能与重要机制已整理成可查阅、可回查的参照知识库；它不是一份“Pi所有功能生产可用”或“所有历史材料逐行审计”的认证。
