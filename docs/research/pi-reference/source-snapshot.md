...
文中本机链接使用 `/Users/ayu/Learn/pi/…:行号`，行号只对应上面的 commit。换机器时把前缀换成自己的 Pi checkout；文件内的**符号名**比行号更稳定。
...
以后带读先核对 HEAD；不一致时对照 `git diff <研究commit> -- <相关路径>`，更新受影响结论即可，不必重新扫描全仓。不要为了匹配文档直接切用户工作树。

## 全仓覆盖的含义

目标是覆盖仓库全部功能域、入口、持久化与协议边界、平台适配、扩展样例和工程工具，并把重要机制整理为可带读资料。文件清单用于发现遗漏；模块覆盖记录用于证明具体研究内容。清单中的文件存在、测试数量或源码行数，均不单独证明已读懂。
