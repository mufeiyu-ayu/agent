...
每步写明：做什么、证明完成、AI 动手前查的 Pi 素材。先后与触发看 workbench 第 7 节，这里不重复。

### R0 保持当前链路可运行（已完成）

先由 #118 / #119 / #120 整理，再由 #115（模型请求重试与 Loop 上限，2026-09-18 合并）、#116（同轮文本 + 顺序多 Tool Call，2026-09-19 合并）完成本步；#117（Responses adapter）2026-09-18 关闭转 Gated。参照 Pi 的多 content block 和 request/attempt 区分，保留当前模型/工具/前端独立契约。
...
「何时值得加入」只细化 workbench 第 7 节「后」行的触发，冲突时以第 7 节为准。这些候选没有日历承诺，不自动立项。Skill 限定为版本化文本与资源引用，按低信任输入处理，不扩大工具权限。MCP 是外部能力的连接边界，服务器会执行代码：可接入服务器、凭据、授权主体、网络访问与副作用分别限制，先支持平台允许的远端服务，不开放租户自定义 stdio 启动命令；执行不可信租户 JS 仍不做。多 Agent、MCP marketplace、完整 Chord 移植仍不属于这条最小主线。

## 二、规模参照（估算，不是承诺）

各步相对规模，一人配合 AI 全职；档级估算以 workbench 第 7 节为准，本表只作 Pi 对应的参照。没有固定产品边界与权重模型，所以不给完成度百分比、不做合计，也不用于判断学习是否完成；需要排期时用已完成同类任务的实际速度校准。
