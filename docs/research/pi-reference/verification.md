...
- **机械校验**：脚本逐条打开全部 `/Users/ayu/Learn/pi/...:行号` 与 `/Users/ayu/Desktop/agent/...:行号` 链接（含图表 JSON），核对文件存在、行号在范围内、链接文字里的符号出现在锚点附近。当时 529 条，0 条失效，相对链接 0 条失效（2026-09-15 记录；2026-09-23 起 Pi 位置改写为仓库相对路径、本仓库文件改相对链接，这条脚本口径不再直接适用）。
