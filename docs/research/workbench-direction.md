...
| 共享知识库 | 对话产物一键存为知识库文档，走现有 Article 索引；agent 检索按当前用户可见范围过滤 | 用户说的「共享记忆」就是这一条，不做独立 Memory 子系统 |
...
| 独立 Memory 子系统做「共享记忆」 | 团队知识库加现有 Grounded Retrieval 已覆盖；长期 Memory 仍按能力缺口规则等真实卡住 |
