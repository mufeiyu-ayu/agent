...
> 本页是首轮 13 个架构域闭环后的阶段性快照，不是当前任务的最终停止声明。用户已明确要求周额度剩余低于 50% 才停止；后续深挖批次持续记录在 [research-progress.md](./research-progress.md)，最终额度与校验结论将在真正收尾时覆盖本页。
...
- P1 后：剩余 97%。
- 学习指南后：剩余 96%。
- 最终收尾前：剩余 96%；校验后为 95%；Ready PR 远程复核后的最终读数为 94%。

以上读数均高于 50%，没有触发额度停止。Ready PR 只是可审查的阶段性载体，不构成研究终点；后续继续按 Issue 的源码证据标准深挖，并在每批后重新采样。
...
首轮一次性校验结果：原有路径 `55/55` 保留；研究 Markdown `57` 个且全部从根 README 可达；82 个 literal 本地引用与 240 个全文 Codex 路径 token 均存在；Mermaid `17` 个且 fence 闭合；18 个必需架构术语齐全；14 个学习模块结构完整；变更文件全部位于 `docs/research/**`。后续批次必须重复必要校验，不能沿用此结果冒充最终结果。

真正最终的命令与结果以停止条件满足后的 PR 描述为准。
