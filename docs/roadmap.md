...
项目已经完成从固定字段 SEO 生成器到 Session Chat 的迁移，并接入了流式输出基础链路。接下来不要直接跳到 RAG 或多 Agent，而是先把一次用户输入建模成可追踪、可中断、可恢复的 Agent Run。
...
| 阶段 3：Streaming Chat | 收口中 | 流式输出、停止生成、最终态一致 | NDJSON stream、`ABORTED` 状态 | `done/error/aborted` 不残留 `STREAMING` |
| 阶段 4：Agent Runtime 基础 | 当前下一阶段 | 记录一次 Agent 运行过程并抽 runtime 边界 | `AgentRun`、`AgentStep`、Runtime service | 每次发送都有 run/step，可测试可追踪 |
...
| 优先级 | 任务 | 说明 |
| --- | --- | --- |
| P0 | 阶段 3 收口 | 先确认停止生成、错误、完成三种最终态稳定 |
| P0 | 阶段 4 Task 1 | 实现 `AgentRun` / `AgentStep` 基础模型 |
| P1 | Runtime 边界 | 从 `SeoService.chatStream()` 中拆出运行时编排 |
