...
Admin Console Task 0 已完成实现并通过验收，建立了独立的 `apps/admin` 后台前端基础壳。Task 1 的静态 Run List / Run Detail UI 已实现、待验收，当前只使用前端类型化 Mock，不包含 Run / Step 查询 API、真实运行数据或权限能力。这些 UI 任务不代表阶段 8 已启动或完成；阶段 6、阶段 7、阶段 8 的既有状态保持不变。
...
Admin Console 的任务边界与进度以 [tasks/admin-console.md](./tasks/admin-console.md) 为准；Task 0 已 Completed，Task 1 为 Active 且处于“已实现、待验收”，Task 2-4 均为 Planned。
...
| P1 | Admin Console Task 1 学习验收 | 验收静态 Run List / Run Detail 的数据语义、安全投影和视觉效果；保持 Draft，不推进 Task 2 |
