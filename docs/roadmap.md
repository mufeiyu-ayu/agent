...
Admin Console Task 0 与 Task 1 已完成并通过验收：项目已经具备独立的 `apps/admin` 后台前端基础壳，以及使用类型化 Mock 实现的静态 Run List / Run Detail、Trace、Messages 和 Safe Raw Data。当前仍没有 Run / Step 查询 API、真实运行数据接入或权限能力，因此这些 UI 任务不代表阶段 8 已启动或完成；阶段 6、阶段 7、阶段 8 的既有状态保持不变。
...
| P1 | 规划 Admin Console Task 2 | 结合已经确认的 Run List / Run Detail 信息架构，定义只读 Run / Step 查询 API、安全 DTO 和分页筛选边界；开始前单独创建 Issue |
...
| 暂不做 | 原因 |
| --- | --- |
| 直接把 Mock UI 接入数据库 | 需要先通过独立 Task 2 定义安全的只读 API contract |
