...
Admin Console Task 0 已通过 Issue #19 启动，用于建立 `apps/admin` 后台前端基础壳。它只是阶段 8 可观测性 UI 的基础设施，不包含 Run / Step 查询 API、真实运行数据或权限能力，不代表阶段 8 已启动或完成；阶段 6、阶段 7、阶段 8 的既有状态保持不变。
...
Admin Console 的任务边界与进度以 [tasks/admin-console.md](./tasks/admin-console.md) 为准；当前仅 Task 0 为 Active，Task 1-4 均为 Planned。

## 当前优先级

| 优先级 | 任务 | 说明 |
| --- | --- | --- |
| P0 | Admin Console Task 0 | `apps/admin` 基础壳已实现、待验收；不接真实 Run / Step 数据 |
