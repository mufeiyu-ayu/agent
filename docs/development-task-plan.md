...
当前主线已从“固定字段 SEO 生成器”调整为“自然语言 SEO Agent 聊天助手”：

```txt
用户输入自然语言问题
  -> Vue 前端调用 Nest API
  -> Nest 后端组织 system / user messages
  -> LLM 按 SEO Agent 角色自然回复
  -> 前端以对话 turn 展示 user / assistant
```
...
| T17 | 流式输出 | 实现 SEO 生成的流式体验 | 后端通过 SSE 事件把生成过程状态和最终结构化结果返回前端 | Streaming、SSE、chunk 消费、前端增量渲染 | 用户能看到生成过程，而不是只等待最终结果；非流式接口仍保留可用 | 暂缓 | 当前主链路已改回普通 POST 聊天接口，SSE 从第一版中下线；后续需要真实 token 级或 trace 级 streaming 时再恢复 |
...
| T19 | 简单会话上下文 | 支持一次生成任务内的上下文组织 | 后端能组织 system / user / assistant message，必要时保留单次任务上下文 | messages 组织、上下文管理、token 控制 | 生成流程中的 messages 结构清晰；不会把无关历史无限追加进模型上下文 | 进行中 | 已完成最小 `system + user` 单轮 SEO Agent chat，下一步再把最近对话 history 有控制地加入 messages |
| T20 | 产品收尾 | 完成 AI SEO Agent 第一版体验闭环 | 前端、后端、LLM、SEO 建议、会话结果和错误反馈形成完整可演示产品 | 端到端 Agent 应用组织、产品验收、作品集整理 | 新用户按 README 启动后，可以完成一次完整 SEO 生成流程；README 有清晰演示说明 | 未开始 | 等 T19 多轮 history 稳定后，再整理第一版演示说明和验收清单 |
...
```txt
T17 事件级 SSE 流式体验已完成；T18 Tool Calling 和 T19 简单上下文仍未开始。
```
