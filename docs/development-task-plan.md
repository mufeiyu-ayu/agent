...
| T09 | JSON Output 落地 | 让模型稳定返回结构化 SEO JSON | Prompt 约束输出字段，代码解析并校验模型结果 | JSON Output、结构化 prompt、运行时校验、模型输出不可信原则 | 模型返回非法 JSON 或字段缺失时，后端返回明确错误；合法时输出 `title`、`description` 等结构化字段 | 进行中 | 已新增 prompt、output validator 和 `SeoGenerationService`；下一步决定是否直接进入 T10 接入 `SeoService` |
