...
| 用户表达 | Skill | 默认完成点 |
| --- | --- | --- |
| “完成 Issue #N”“读取 Issue #N 并实现” | `github-issue-workflow` | Draft PR、验证结果、学习交接；状态为已实现、待验收 |
...
| 本地 Codex | 读取 Issue、创建分支、实现、验证、提交、Draft PR、Review 修复和学习交接 |
| 云端 Codex Review | PR 创建后自动检查高风险问题 |
| 用户 | 最终确认验收、修复范围、Ready、合并和分支清理 |
...
“完成 Issue #N”默认授权到 Draft PR，但不授权 Ready、合并或最终任务收口。实现阶段可以更新 checklist、验证结果和交付链接，但不得：
...
状态收口不等于授权合并。除非用户同时明确授权，PR 继续保持 Draft。
...
- Draft 或未合并分支继续保留。
- 用户明确授权“合并 PR”即同时授权将该 PR 从 Draft 转为 Ready；不单独重复确认这一步。
- 放弃任务的分支只有用户确认后才删除。
...
- 正式任务状态、roadmap、`AGENTS.md` 或 skills 变更：建议仍先创建 Issue，再走分支和 Draft PR。
