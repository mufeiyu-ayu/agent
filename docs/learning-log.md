...
| 2026-06-04 | 运行时配置 | 将 DeepSeek 模型列表和余额查询接入后端代理，再在前端作为 Agent 运行时设置展示。 | 模型选择、成本/余额可见性、后端代理模型平台接口、前端不保存 API Key | `apps/api/src/llm/llm.controller.ts`、`apps/api/src/llm/llm.service.ts`、`apps/web/src/hooks/useLlmRuntime.ts`、`apps/web/src/api/llm.ts` | `/api/llm/models` 返回 `deepseek-v4-flash` 和 `deepseek-v4-pro`；`/api/llm/balance` 返回账户可用状态和余额信息；生成请求可以携带当前模型。 | 模型运行时配置也是 Agent 产品的一部分：用户需要知道当前用哪个模型、账户是否可用；但 API Key 仍必须留在后端。 | 下一步进入 T19，把当前输入、模板上下文和最近对话历史整理成传给模型的 `messages`。 |
| 2026-06-15 | SEO Agent 角色基线 | 将固定字段 SEO 生成链路退回为普通聊天链路：system prompt 定义 SEO Agent 角色，user message 直接驱动回复。 | system prompt、角色设定、普通 chat messages、从字段化任务回到自然语言交互 | `apps/api/src/seo/prompts/seo-agent.prompt.ts`、`apps/api/src/seo/seo.service.ts`、`apps/web/src/hooks/useSeoWorkspace.ts` | `POST /api/seo/chat` 使用 `{ message, model }` 返回普通文本 `reply`；前端只维护 user / assistant turn；`pnpm typecheck` 和 `pnpm lint` 通过。 | 过早字段化会把 Agent 限制成表单生成器；当前阶段先把“SEO Agent 角色稳定 + 自然对话主链路”打稳，再逐步加 history、结构化输出和工具调用。 | 下一步练习多轮上下文：把最近几轮 user / assistant 作为 history 有控制地传给模型。 |
