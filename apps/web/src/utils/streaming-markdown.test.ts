...
it('刚到达、还没有正文的 opener 先隐藏', () => {
  assert.equal(completeStreamingMarkdown('先说 **'), '先说 ')
  assert.equal(completeStreamingMarkdown('先说 `'), '先说 ')
  assert.equal(completeStreamingMarkdown('先说 **结论**'), '先说 **结论**')
})

it('列表标记、词内下划线、乘号与转义不当作强调', () => {
...
it('闭合符紧贴正文，切在空白后也不会退化成字面标记', () => {
  assert.equal(completeStreamingMarkdown('先说 **结论 '), '先说 **结论**')
  assert.equal(completeStreamingMarkdown('**见图 ![示意'), '**见图**')
  assert.equal(completeStreamingMarkdown('a `b '), 'a `b `')
})

it('图片隐藏不跨行，链接没有 href 内容时不补右括号', () => {
...
it('补齐以行与表格单元格为边界，前面行里未闭合的字面标记不补到末尾', () => {
  for (const text of [
    '- 匹配 *.ts 文件\n- 其他',
    '- 2**10 很大\n- 其他',
    '| 模式 | 说明 |\n|---|---|\n| *.ts | TS 文件',
...
it('#169 AC-02 当前行里多半是字面字符的 opener 不补：2*3、2**10、*.ts、行内公式里的 x*y', () => {
...
it('#169 AC-03 表格：解析器认定的表格每行按单元格切分，代码段里的 \\| 不是边界', () => {
...
it('#169 AC-04 尾块末尾只有 ATX 标题标记时先不显示', () => {
  for (const tail of ['#', '##', '## ', '> ###', '- ##', '1. #'])
