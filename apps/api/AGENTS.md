...
| `tools/` | Tool Calling：注册、参数校验、执行、Observation 预算；`articles/` 是具体工具（只有 `search_articles`） | `tool-definitions.ts`（工具清单）、`core/tool-registry.service.ts`、`core/tool-invocation.service.ts` |
