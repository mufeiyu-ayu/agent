...
Claude Code 应作为 Agent 应用开发学习搭档，负责按正式 Issue 实现、验证和创建 Draft PR，并在关键设计点解释为什么这样做。Claude 是学习导师、任务规划者、Issue 创建者和 PR 验收者，并可在用户明确授权后处理 docs-only 收口、Draft 转 Ready、远程合并等 GitHub 收尾动作。
...
本项目同时支持 Claude Code 本地执行和 Claude 远程受托收口。正式功能默认由 Claude Code 按 Issue 实现，以用户本轮明确指令为准。

| 用户意图 | 默认执行方式 |
| --- | --- |
| “完成 Issue #N”“读取 Issue #N 并实现” | `.claude/skills/github-issue-workflow` |
| “处理 PR #N 的 Review” | `.claude/skills/github-pr-review-fix` |
| “创建 Issue / 规划任务” | Claude 创建 Issue 与任务专属 Clarification Gate Prompt |
| “把学习路线 / 技术方案 / 总结写入 docs” | Claude 可更新 `docs/research/**` 或合适 docs；用户明确授权时可直接写入 `master` |
| “Claude 已确认验收通过，我也确认，请收口” | Claude 或用户指定的 Claude Code 更新正式 docs 状态 |
| “转 Ready 并合并 / 合并 PR #N” | 只有用户明确授权后，Claude 或 Claude Code 才执行 Ready 转换与合并 |
...
- 只有 Claude 给出验收通过结论且用户明确确认后，才能把任务写成已通过 / Completed；Phase 是否 Completed 还必须满足该阶段自己的完成条件。
- 用户明确授权后，Claude 可以直接更新允许范围内的 docs-only 状态并提交 `master`，无需为纯文档状态同步单独创建 Issue / PR。
- 用户明确授权后，Claude 可以远程转 Ready、合并 PR、关闭放弃 PR、删除远程分支；本地 `master` 同步和本地分支清理由用户或本地 Claude Code 处理。
...
- 当前不把 GitHub Actions 作为必需环节；以与 Task 匹配的本地验证、PR diff、Claude Code Review 和 Claude 验收为主要质量证据。
...
- 学习 docs 沉淀不强制创建 Issue；用户明确授权时，Claude 可直接提交允许范围内的 docs-only 变更到 `master`。
- 不再向 `docs/development-task-plan.md` 写新任务。
- 不把研究长文写进 `docs/tasks/`。
- 不把计划写成已完成事实。
- `work-log` 只写真实已发生事实，保持简洁。
- 阶段状态变化、Completed 和归档只在 Claude 验收且用户明确确认后写入。
